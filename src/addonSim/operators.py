...
class MW_gen_OT_(types.Operator):
    ...
    def execute(self, context: types.Context):
        ...
        self.stats.log(f"retrieved points: {len(points)}")
        if not points:
        ...
        self.stats.log(f"retrieved shape data: faces {len(faces4D)} (r {bb_radius:.3f})")
        ...
        self.stats.log(f"transform/limit points: {len(points)} (rnd {cfg.source_noise})")
